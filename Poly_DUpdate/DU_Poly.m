function [S, XX, DS, vP, RE, Conv, Conv2, Conv3, Conv4, PofX] = DU_Poly(A,X0,grph,minp,maxiter)
...
%     delta = min(diag(A(:,:,end)*X0 + A(:,:,end)))
%     delta = min(diag(sum(A(:,:,3:end),3)*X0 + sum(A(:,:,3:end),3)))
    delta = min([diag(DU_polynom(X0,A));min(min((Polynom(X0,A) - A(:,:,2)*X0)./X0))])
%     delta = min(min(DU_polynom(X0,A)))
%     delta = min(diag(Polynom(X0,A(:,:,3:end))*X0 + sum(A(:,:,3:end),3)))
    eig(-(A(:,:,2) + delta*I))
    Polynom(X0,A) - A(:,:,2)*X0 - delta*X0
    X1 = -(A(:,:,2) + delta*I)\(Polynom(X0,A) - A(:,:,2)*X0 - delta*X0);
%     X1 = -(A(:,:,2) + delta*A(:,:,end))\(Polynom(X0,A) - A(:,:,2)*X0 - delta*A(:,:,end)*X0);
