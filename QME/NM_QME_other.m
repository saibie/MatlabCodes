function [S, XX, H, D_X0, DS, vQ, RE, Conv, Conv2, Conv3, Conv4] = NM_QME_other(A,B,C,X0,minp,maxiter)
...
while (pert > minp) & (i < maxiter)
    %fprintf('i : %d\n',i)                                                   % Experiment
    D_X0{i} = kron(I,B + A*X0) + kron(X0',A);                      % D_X
    bbb = max(diag(-D_X0{i}));
    N = eye(n^2) + D_X0{i}./bbb;
