function [S, XX, H, D_X0, DS, vQ, RE, Conv, Conv2, Conv3, Conv4] = NM_QME_Advanced(A,B,C,X0,minp,maxiter)
...
while (pert > minp) & (i < maxiter)
    %fprintf('i : %d\n',i)                                                   % Experiment
    D_X0{i} = kron(I,A) - kron(inv(X0)',C/X0);                      % D_X
    vQ{i} = -reshape(A*X0 + B + C/X0,n^2,1);
    vH = D_X0{i} \ vQ{i};
    %vH = pinv(D_X0{i}) * vQ{i};
    H{i} = reshape(vH,n,n);
    X1 = X0 + H{i};
...
if i >= maxiter                                                             % The Iteration over the Max Iteration
    ...
    S = X1;
    DS = kron(I,A) - kron(inv(S)',C/S);                      % D_X
    XX = X;
    for j=1:i-1
