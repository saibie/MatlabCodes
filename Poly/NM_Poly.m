function [S, XX, H, P_X0, DS, vP, RE, Conv, Conv2, Conv3, Conv4, PofX] = NM_Poly(A,X0,grph,minp,maxiter)
...
%     normP = norm(A(:,:,1),'fro');
%     for j=1:n-1
%         normP = normP + norm(A(:,:,j+1),'fro')*norm(X1,'fro')^j;
%     end
    
%     X2 = X0 + 2*H{i};%======== 2¹è Àû¿ë =============
%     normP2 = 0;
%     for j=1:n-1
%         normP2 = normP2 + norm(A(:,:,j+1),'fro')*norm(X2,'fro')^j;
%     end
%     normP2 = normP2 + norm(A(:,:,1),'fro');
%     if norm(Polynom(X2,A),'fro')/normP2 <= minp
%         X1 = X2;
%     end%============================================
