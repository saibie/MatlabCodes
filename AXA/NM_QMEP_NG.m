function [S, XX, H, D_X0, DS, vQ, RE, Conv, Conv2, Conv3, Conv4] = NM_QMEP_NG(n,delta,minp,maxiter)
...
for l=2:n
    D(l,l) = max(roots([1-sqrt(delta) 3*n-2-sqrt(delta) 1-(3*n-2)*sqrt(delta)]));
end
S1 = P*D*inv(P);
...
if i >= maxiter                                                             % The Iteration over the Max Iteration
    ...
    Conv2 = 0;
    for j=1:(maxiter - 2)
        Conv3(j) = norm((X{j+1} - S1),'fro') / (norm((X{j+1}),'fro'));
    end
    for j=1:(maxiter - 2)
    ...
    XX = X;
    for j=1:i-1
        Conv(j) = norm((X{j+1} - S1),'fro') / norm((X{j} - S1),'fro');
        Conv2(j) = norm((X{j+1} - S1),'fro') / (norm((X{j} - S1),'fro')^2);
        Conv3(j) = norm((X{j} - S1),'fro') / (norm(S1,'fro'));
        Conv4(j) = norm(Qu(X{j+1},A,B,C),'fro');
    end
