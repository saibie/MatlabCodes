function [S, XX, H, D_X0, DS, vQ, RE, Conv, Conv2, Conv3, Conv4] = NM_AXA(A,C,X0,minp,maxiter)
...
if ~((nA(1)==nA(2))&(nC(1)==nC(2))&(nX0(1)==nX0(2)))         % Are they square matrices?
    disp('There is a non-square matrix in A, B, C and X0. (by AXA)');
    return
elseif ~((nA(1)==nC(1))&(nC(1)==nX0(1)))                     % Are the size of matrices same?
    disp('The size of A,B,C and X0 are not same. (by AXA)');
    return
else
    n = nA(1);
end
...
while (pert > minp) & (i < maxiter)
    ...
    normX1 = norm(X1,'fro');
    pert = norm(Axa(X1,A,C),'fro')/(normX1^2 + normA^2*normX1 + normC);
    %pert = norm(Axa(X1,A,C),'fro');
...
if i >= maxiter                                                             % The Iteration over the Max Iteration
    ...
    for j=1:(maxiter - 2)
        Conv4(j) = norm(Axa(X{j+1},A,C),'fro');
    end
else                                                                        % Conver
    S = X1;
