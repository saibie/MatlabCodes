function [S, XX, H, D_X0, DS, vQ, RE, Conv, Conv2, Conv3, Conv4] = NM_QPD(A,X0,minp,maxiter)
...
if ~((nA(1)==nA(2))&(nX0(1)==nX0(2)))         % Are they square matrices?
    disp('There is a non-square matrix in A and X0. (by QPD)');
    return
elseif ~((nA(1)==nX0(1)))                     % Are the size of matrices same?
    disp('The size of A and X0 are not same. (by QPD)');
    return
else
    n = nA(1);
end
...
while (pert > minp) & (i < maxiter)
    %fprintf('i : %d\n',i)                                                   % Experiment
    D_X0{i} = kron(I,X0) - kron(A',A') + kron(X0',I);                      % D_X
    vQ{i} = reshape((-(Pd(X0,A))),n^2,1);
    vH = D_X0{i} \ vQ{i};
    H{i} = reshape(vH,n,n);
    X1 = X0 + H{i};
    X{i+1} = X1;
    ...
    normX1 = norm(X1,'fro');
    pert = norm(Pd(X1,A),'fro')/(normX1^2 + normA^2*normX1);
...
if i >= maxiter                                                             % The Iteration over the Max Iteration
    ...
    for j=1:(maxiter - 2)
        Conv4(j) = norm(Pd(X{j+1},A),'fro');
    end
else                                                                        % Conver
    S = X1;
    DS = kron(I,S)+kron(A',A')+kron(S',I);
    XX = X;
    for j=1:i-1
