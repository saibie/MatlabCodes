function [S, XX, H, D_X0, DS, vQ, RE, Conv, Conv2, Conv3] = NM_QME2(A,B,C,X0,offplot,minp,maxiter)
...
while (pert > minp) & (i < maxiter)
    ...
    H{i} = reshape(vH,n,n);
    X1 = X0 + H{i};
    X2 = X0 + 2*H{i};                                                       % If D_S is singular, it plays a role.
    X{i+1} = X1;
    if i == 1
        RE(i) = 0;
    else
