function [S, H, T] = Halley_MME0(n,A,B,C,X0,minp,maxiter)
...
while (pert > minp) & (i < maxiter)
    ...
    P1_X0 = -kron(I,B);                      % D_X
    for iii=0:n-1
        P1_X0 = P1_X0 + kron((X0')^(n-1-iii), A*X0^iii);
    end
    
    P11_X0P_X0 = zeros(m^2);
    for iii=0:n-2
        for jjj=0:n-2-iii
            P11_X0P_X0 = P11_X0P_X0 + kron((X0')^(n-2-iii-jjj),A*X0^(iii)*P_X0*X0^(jjj)) + kron((X0^(jjj)*P_X0*X0^(n-2-iii-jjj))',A*X0^iii);
        end
    end
    
    vP = reshape(-(P_X0),m^2,1);
    vH = (2*(P1_X0)^2 - P11_X0P_X0) \ (2*P1_X0*vP);
    H{i} = reshape(vH,m,m);
    X1 = X0 + H{i};
%     X{i+1} = X1;
%     if i == 1
