function [S, XX, H, D_X0, DS, vQ, RE, Conv, Conv2, Conv3, Conv4] = NM_AXB(p,A,B,C,X0,minp,maxiter)
...
while (pert > minp) & (i < maxiter)
    %fprintf('i : %d\n',i)                                                   % Experiment
    SSS = 0;for j=1:p;SSS = SSS + kron(X0'^(p-j),X0^(j-1));end
    D_X0{i} = SSS - kron(B',A);                      % D_X
    vQ{i} = reshape((-(AXB(p,X0,A,B,C))),n^2,1);
    vH = D_X0{i} \ vQ{i};
    H{i} = reshape(vH,n,n);
    X1 = X0 + H{i};
    X{i+1} = X1;
    ...
    normX1 = norm(X1,'fro');
    pert = norm(AXB(p,X1,A,B,C),'fro')/(normA*normX1^2 + normB*normX1 + normC);
    %pert = norm(AXB(p,X1,A,B,C),'fro');
...
if i >= maxiter                                                             % The Iteration over the Max Iteration
    ...
    for j=1:(maxiter - 2)
        Conv4(j) = norm(AXB(p,X{j+1},A,B,C),'fro');
    end
else                                                                        % Conver
    S = X1;
    SSS = 0;for j=1:p;SSS = SSS + kron(S'^(p-j),S^(j-1));end
    DS =  SSS - kron(B',A);
    XX = X;
    for j=1:i-1
